...
import { User, SystemSettings, Student } from '../types';
import { useNavigate } from 'react-router-dom';
import ThreeOrb from '../components/ThreeOrb';
import { QrCode, Smartphone, Copy, Check, Shield, ArrowRight, Search, User as UserIcon, AlertCircle, Lock } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-white relative font-sans pt-32 pb-16 px-4 flex justify-center">
             ...
             <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                ...
                <div className="bg-white text-gray-900 rounded-2xl shadow-2xl overflow-hidden relative flex flex-col min-h-[500px]">
                    {submitted ? (
                        ...
                        <div className="p-8 flex-1 flex flex-col">
                            <div className="mb-6 pb-6 border-b border-gray-100 flex items-center justify-between">
                                <h3 className="text-xl font-bold flex items-center gap-2 text-gray-800">
                                    {settings.paymentMode === 'manual' ? <QrCode className="text-indigo-600"/> : <Smartphone className="text-purple-600"/>}
                                    {settings.paymentMode === 'manual' ? 'Scan & Pay' : 'Online Gateway'}
                                </h3>
                            ...
                            {settings.paymentMode === 'manual' ? (
                                <div className="space-y-6 flex-1 flex flex-col">
                                    <div className="flex justify-center py-4">
                                        <div className="p-4 bg-white border-2 border-indigo-50 rounded-2xl shadow-[0_0_20px_rgba(0,0,0,0.05)]">
                                            <img 
                                                src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(`upi://pay?pa=${settings.adminUpiId}&pn=ShriyasCoaching&cu=INR`)}`}
                                                alt="UPI QR Code" 
                                    ...
                                    <div className="bg-indigo-50 p-4 rounded-xl flex items-center justify-between border border-indigo-100">
                                        <div className="overflow-hidden mr-2">
                                            <p className="text-[10px] text-indigo-500 font-bold uppercase tracking-wider mb-1">Coach UPI ID</p>
                                            <p className="text-sm md:text-base font-mono text-indigo-900 font-bold truncate select-all">{settings.adminUpiId}</p>
                                        </div>
                                        <button onClick={handleCopy} className="text-indigo-600 hover:text-indigo-800 p-2 bg-white rounded-lg shadow-sm hover:shadow transition-all">
                                ...
                                <div className="flex-1 flex flex-col justify-center items-center text-center space-y-8">
                                    ...
                                    <div>
                                        <h3 className="text-xl font-bold text-gray-800 mb-2">Pay via PhonePe Gateway</h3>
                                        <p className="text-gray-500 text-sm max-w-xs mx-auto leading-relaxed">Secure instant payment using UPI, Credit Card, or Netbanking.</p>
                                    </div>
